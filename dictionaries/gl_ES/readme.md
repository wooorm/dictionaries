...
Galician (Spain) spelling dictionary in UTF-8.
...
```bash
npm install dictionary-gl-es
```

## Usage

```js
var glES = require('dictionary-gl-es');

glES(function (err, result) {
    if (err) throw err;
...
var base = require.resolve('dictionary-gl-es');
...
Dictionary and affix file: [GPL-3.0](https://github.com/wooorm/dictionaries/blob/master/dictionaries/gl_ES/LICENSE).  Rest: MIT ©
[Titus Wormer][home].
...
[source]: http://extensions.openoffice.org/en/project/corrector-ortografico-hunspell-para-galego
