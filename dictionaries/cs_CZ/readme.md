...
Czech (Czech Republic) spelling dictionary in UTF-8.
...
```bash
npm install dictionary-cs-cz
```

## Usage

```js
var csCZ = require('dictionary-cs-cz');

csCZ(function (err, result) {
    if (err) throw err;
...
var base = require.resolve('dictionary-cs-cz');
...
Dictionary and affix file: [GPL-2.0](https://github.com/wooorm/dictionaries/blob/master/dictionaries/cs_CZ/LICENSE).  Rest: MIT ©
[Titus Wormer][home].
