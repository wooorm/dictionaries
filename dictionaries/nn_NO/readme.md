...
Norwegian Nynorsk (Norway) spelling dictionary in UTF-8.
...
```bash
npm install dictionary-nn-no
```

## Usage

```js
var nnNO = require('dictionary-nn-no');

nnNO(function (err, result) {
    if (err) throw err;
...
var base = require.resolve('dictionary-nn-no');
...
Dictionary and affix file: [GPL-2.0](https://github.com/wooorm/dictionaries/blob/master/dictionaries/nn_NO/LICENSE).  Rest: MIT ©
[Titus Wormer][home].
